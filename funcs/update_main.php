...
//Updates page for main
	...
	$sqlUpdate = "UPDATE `main` SET `title`='". $title."', `head`='". $head."', `description`='". $description."', `content`='". $content."' WHERE `name`='".$_GET['admin']."'";
	@$resUpdate = mysql_query($sqlUpdate) or die(mysql_error());
	echo $sqlUpdate;
	header('Location: ../acp.php?mode=edit&name='.$_GET['admin']);
}
